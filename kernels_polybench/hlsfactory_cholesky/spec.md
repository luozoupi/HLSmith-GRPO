...
- Benchmark name: `hlsfactory_cholesky`.
- Required HLS wrapper top function: `kernel_cholesky`.
- Include `cholesky.h` exactly once and reuse its declarations.
- Header-declared functions available for reuse: `kernel_cholesky`.
- Functions already defined in the plain input whose names/signatures should be preserved unless wrapping is required: `kernel_cholesky`.
