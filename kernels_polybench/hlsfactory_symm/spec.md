...
- Benchmark name: `hlsfactory_symm`.
- Required HLS wrapper top function: `kernel_symm`.
- Include `symm.h` exactly once and reuse its declarations.
- Header-declared functions available for reuse: `kernel_symm`.
- Functions already defined in the plain input whose names/signatures should be preserved unless wrapping is required: `kernel_symm`.
