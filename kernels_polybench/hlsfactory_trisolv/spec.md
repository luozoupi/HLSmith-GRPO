...
- Benchmark name: `hlsfactory_trisolv`.
- Required HLS wrapper top function: `kernel_trisolv`.
- Include `trisolv.h` exactly once and reuse its declarations.
- Header-declared functions available for reuse: `kernel_trisolv`.
- Functions already defined in the plain input whose names/signatures should be preserved unless wrapping is required: `kernel_trisolv`.
