...
- Benchmark name: `hlsfactory_atax`.
- Required HLS wrapper top function: `kernel_atax`.
- Include `atax.h` exactly once and reuse its declarations.
- Header-declared functions available for reuse: `kernel_atax`.
- Functions already defined in the plain input whose names/signatures should be preserved unless wrapping is required: `kernel_atax`.
