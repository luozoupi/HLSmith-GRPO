...
- Benchmark name: `hlsfactory_gemver`.
- Required HLS wrapper top function: `kernel_gemver`.
- Include `gemver.h` exactly once and reuse its declarations.
- Header-declared functions available for reuse: `kernel_gemver`.
- Functions already defined in the plain input whose names/signatures should be preserved unless wrapping is required: `kernel_gemver`.
