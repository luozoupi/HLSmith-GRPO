...
- Benchmark name: `hlsfactory_lu`.
- Required HLS wrapper top function: `kernel_lu`.
- Include `lu.h` exactly once and reuse its declarations.
- Header-declared functions available for reuse: `kernel_lu`.
- Functions already defined in the plain input whose names/signatures should be preserved unless wrapping is required: `kernel_lu`.
